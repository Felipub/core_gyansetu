...
if ($isLoggedIn) {
    ...
    if ($cacheLoad || !$session->has('menuMainItems')) {
        ...
        // GS: Hide some navigation Menu for teachers
        //GS// //$role = getRoleCategory($session->get('gibbonRoleIDCurrent'), $connection2); //GS//
        $roleList = getRoleList($session->get('gibbonRoleIDCurrent'), $connection2); //GS//
        //DEBUG//die(var_dump($roleList[0][1])); //GS//
        $role = $roleList[0][1]; //GS//
        //DEBUG//die(var_dump($menuMainItems)); //GS//
        ...
        foreach ($menuMainItems as $category => &$items) {
            ...
            foreach ($items as &$item) {
                ...
                if ($role == 'Teacher' && $item['name'] == "Students") { //GS//
                    unset($item); //GS-NOT WORKING//
                    continue; //GS//
                } //GS//
...
/*
        if ($role == 'Teacher') //GS//
        { //GS//
            unset($menuMainItems['Admin']); //GS//

            foreach ($menuMainItems['People'] as $item)
            {
               if ($item['name'] == "Students" || $item['name'] == "Attendance")
                {
                    //die(var_dump($item));
                    unset($menuMainItems['People'][$item]); //GS//
                }
            }
        } //GS//
