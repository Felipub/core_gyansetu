            ...
            if ($result->rowCount() != 1) {
                ...
                if ($result2->rowCount() != 1) {
                    ...
                    if ($values['groupingID'] != '' and $values['gibbonPersonIDCreator'] != $_SESSION[$guid]['gibbonPersonID']) {
                        ...
                        //GS//$row = $form->addRow();
                        //GS//    $row->addLabel('courseName', __('Class'));
                        //GS//    $row->addTextField('courseName')->required()->readOnly()->setValue($course['course'].'.'.$course['class']);

                        //GS//$data = array('gibbonCourseClassID' => $gibbonCourseClassID);
                        //GS//$sql = "SELECT gibbonUnit.gibbonUnitID as value, gibbonUnit.name FROM gibbonUnit JOIN gibbonUnitClass ON (gibbonUnit.gibbonUnitID=gibbonUnitClass.gibbonUnitID) WHERE running='Y' AND gibbonCourseClassID=:gibbonCourseClassID ORDER BY name";

                        //GS//$row = $form->addRow();
                        //GS//    $row->addLabel('gibbonUnitID', __('Unit'));
                        //GS//    $units = $row->addSelect('gibbonUnitID')->fromQuery($pdo, $sql, $data)->placeholder();

                        //GS//$data = array('gibbonCourseClassID' => $gibbonCourseClassID);
                        //GS//$sql = "SELECT gibbonUnitID as chainedTo, gibbonPlannerEntryID as value, name FROM gibbonPlannerEntry WHERE gibbonCourseClassID=:gibbonCourseClassID ORDER BY name";
                        //GS//$row = $form->addRow();
                        //GS//    $row->addLabel('gibbonPlannerEntryID', __('Lesson'));
                        //GS//    $row->addSelect('gibbonPlannerEntryID')->fromQueryChained($pdo, $sql, $data, 'gibbonUnitID')->placeholder();
                        ...
                        //GS//$row = $form->addRow();
                        //GS//    $row->addLabel('description', __('Description'));
                        //GS//    $row->addTextField('description')->required()->maxLength(1000);
                        $form->addHiddenValue('description', '-'); //GS//
                        ...
                        //GS//$row = $form->addRow();
                        //GS//    $row->addLabel('viewableStudents', __('Viewable to Students'));
                        //GS//    $row->addYesNo('viewableStudents')->required();
                        $form->addHiddenValue('viewableStudents', 'N'); //GS//

                        //GS//$row = $form->addRow();
                        //GS//    $row->addLabel('viewableParents', __('Viewable to Parents'));
                        //GS//    $row->addYesNo('viewableParents')->required();
                        $form->addHiddenValue('viewableParents', 'N'); //GS//
