...
if (isActionAccessible($guid, $connection2, '/modules/User Admin/user_manage_add.php') == false) {
    ...
    $row = $form->addRow();
        $row->addLabel('preferredName', __('First Name'))->description(__('First name as shown in ID documents.')); //GS//
        $row->addTextField('preferredName')->required()->maxLength(60); //GS//
    ...
    $row = $form->addRow();
        ...
        $row->addUsername('username')
            ->required(); //GS//
            //GS//->addGenerateUsernameButton($form);
    ...
    //GS//    $row->addTextField('address2')->maxLength(255);
    $row = $form->addHiddenValue('address2', ''); //GS//
    ...
    $row = $form->addRow(); //GS//
        $row->addLabel('salary', __('Salary')); //GS//
	$row->addTextField('salary')->maxLength(30); //GS//
