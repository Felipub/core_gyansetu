    ...
    if ($gibbonFamilyID == '') {
        ...
        if ($result->rowCount() != 1) {
            ...
            $row = $form->addRow();
                ...
                $row->addSelectStudent('gibbonPersonID', $_SESSION[$guid]['gibbonSchoolYearID'], array('allStudents' => true, 'byName' => true, 'byRoll' => false, 'showRoll' => true))->placeholder()->required(); //GS//
            ...
            try {
                $dataSelect = array();
                $sqlSelect = "SELECT status, gibbonPersonID, preferredName, surname, username FROM gibbonPerson
                WHERE (gibbonRoleIDPrimary=(SELECT gibbonRoleID FROM gibbonRole WHERE name='Parent')
                OR gibbonRoleIDAll LIKE CONCAT('%',(SELECT gibbonRoleID FROM gibbonRole WHERE name='Parent'),'%'))
                AND (status='Full' OR status='Expected') ORDER BY preferredName, surname, username"; //GS: only parents showing
