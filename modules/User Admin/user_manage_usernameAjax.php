...
/*//GS//    $gibbonRoleID = isset($_POST['gibbonRoleID'])? $_POST['gibbonRoleID'] : '003';
...
//GS-->
    try {
        //$sql = "SELECT MAX(username) AS lastID FROM gibbonPerson WHERE username REGEXP '^[0-9]+$'";
        $sql = "SELECT MAX(CONVERT(username, DECIMAL)) AS lastID FROM gibbonPerson WHERE concat('',username * 1) = username";
        $result = $pdo->executeQuery(array(), $sql);
        //die(var_dump($result->fetchAll()[0]['lastID']));
        if ($result && $result->rowCount() > 0) {
            $nextID = $result->fetchAll()[0]['lastID'];
            echo $nextID+1;
        }
