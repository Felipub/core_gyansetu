...
if (isActionAccessible($guid, $connection2, '/modules/User Admin/user_manage_edit.php') == false) {
    ...
    if ($gibbonPersonID == '') {
        ...
        if ($result->rowCount() != 1) {
            ...
            //Validate Inputs
            if (/*$surname == '' or $firstName == '' or*/ $preferredName == '' or /*$officialName == '' or*/ $gender == '' or $username == '' or $status == '' or $gibbonRoleIDPrimary == '') { //GS//
                $URL .= '&return=error3';
                header("Location: {$URL}");
            } else {
                    ...
                    if (!empty($_FILES['file1']['tmp_name']) or !empty($_FILES['birthCertificateScan']['tmp_name']) or !empty($_FILES['nationalIDCardScan']['tmp_name']) or !empty($_FILES['citizenship1PassportScan']['tmp_name']))
                    {
                        ...
                        if (!empty($_FILES['file1']['tmp_name'])) {
                            ...
                            if (empty($attachment1)) {
                                ...
                                $maxWidth = 360;
                                $maxHeight = 480;
                                //die("width: ".$src_w." - height: ".$src_h);

                                // New crop if needed
                                if ($aspect1 < 1.2) {
                                    $src_w = $height1 / 1.2;
                                    $src_x = ($width1 - $src_w) / 2;
                                    //die("Too Wide! ".$aspect1." - New width: ".$src_w." - New Height: ".$src_h);
                                    //die("Too Wide! ".$aspect1." - New X: ".$src_x." - New Y: ".$src_y);
                                }
                                else if ($aspect1 > 1.4) {
                                    //die("Too High! ".$aspect1);
                                    $src_h = $width1 * 1.4;
                                ...
                                if ($src_w > $maxWidth) {
                                    ...
                                    //die("Too Wide! ".$new_ratio." - New width: ".$dst_w." - New Height: ".$dst_h);
                                }
                                if ($src_h > $maxHeight) {
                                    $new_ratio = $maxHeight / $src_h;
                                    $dst_h = $maxHeight;
                                    $dst_w = $src_w * $new_ratio;
                                    //die("Too Tall! ".$new_ratio." - New width: ".$dst_w." - New Height: ".$dst_h);
                                }
                                ...
                                // imagecopyresized() will take a rectangular area from src_image of width src_w and height src_h at position (src_x,src_y) and place it in a rectangular area of dst_image of width dst_w and height dst_h at position (dst_x,dst_y).
                                ...
                                //die("Source image - ".var_dump($image_name));
                            }
                            //<--GS//
                        }
