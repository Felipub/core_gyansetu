    ...
    if ($gibbonPersonID != '') {
        if ($currentDate > $today) {
            ...
            if (isSchoolOpen($guid, $currentDate, $connection2) == false) {
                ...
                //GS//$row = $form->addRow();
                //GS//    $row->addLabel('reason', __('Reason'));
                //GS//    $row->addSelect('reason')->fromArray($attendance->//GS//())->selected($lastLog['reason'] ?? '');
                $form->addHiddenValue('reason', ''); //GS//

                //GS//$row = $form->addRow();
                //GS//    $row->addLabel('comment', __('Comment'))->description(__('255 character limit'));
                //GS//    $row->addTextArea('comment')->setRows(3)->maxLength(255)->setValue($lastLog['comment'] ?? '');
                $form->addHiddenValue('comment', ''); //GS//
