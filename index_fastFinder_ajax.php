...
if (isset($_SESSION[$guid]) == false or isset($_SESSION[$guid]['gibbonPersonID']) == false) {
        ...
        try {
            ...
            $sql = "SELECT gibbonPerson.gibbonPersonID AS id,
                    (CASE WHEN gibbonPerson.username LIKE :search
                        THEN concat(preferredName, ' ', surname, ' (', gibbonPerson.username, ')')
                        ELSE concat(preferredName, ' ', surname) END) AS name,
                    NULL as type
                    FROM gibbonPerson
    ...
    if ($studentIsAccessible == true) {
        ...
        if ($highestActionStudent == 'View Student Profile_myChildren') {
            ...
            $sql = "SELECT gibbonPerson.gibbonPersonID AS id,
                    (CASE WHEN gibbonPerson.username LIKE :search THEN concat(preferredName, ' ', surname, ' (', gibbonRollGroup.name, ', ', gibbonPerson.username, ')')
                        WHEN gibbonPerson.studentID LIKE :search THEN concat(preferredName, ' ', surname, ' (', gibbonRollGroup.name, ', ', gibbonPerson.studentID, ')')
                        ...
                        ELSE concat(preferredName, ' ', surname, ' (', gibbonRollGroup.name, ')') END) AS name,
                    NULL as type
