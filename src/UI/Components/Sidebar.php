        ...
        if ($_SESSION[$guid]['address'] == '' and isset($_SESSION[$guid]['username'])) {
            ...
            if ($result->rowCount() > 0) {
                ...
                while ($row = $result->fetch()) {
                    ...
                    //GS//echo "<a href='".$_SESSION[$guid]['absoluteURL'].'/index.php?q=/modules/Departments/department_course_class.php&gibbonCourseClassID='.$row['gibbonCourseClassID']."'>".$row['course'].'.'.$row['class'].'</a>';
                    echo $row['course'].'.'.$row['class']; //GS//
                    echo '</td>';
                    if (isActionAccessible($guid, $connection2, '/modules/Planner/planner.php')) {
