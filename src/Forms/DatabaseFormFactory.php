...
class DatabaseFormFactory extends FormFactory
{
    ...
    */
    public function createSelectRollGroup($name, $gibbonSchoolYearID, $all = false, $teacherPersonID = NULL) //GS//
    {
        $data  = array('gibbonSchoolYearID' => $gibbonSchoolYearID);

        //die(var_dump($data));//GS//

        $sql  = "SELECT gibbonRollGroupID as value, name FROM gibbonRollGroup WHERE gibbonSchoolYearID=:gibbonSchoolYearID ";//GS//
        $sql .= ($teacherPersonID ? "AND gibbonPersonIDTutor LIKE '".$teacherPersonID."'" : "");//GS//
        $sql .= " ORDER BY LENGTH(name), name";//GS//
    ...
    public function createSelectUsers($name, $gibbonSchoolYearID = false, $params = array())
    {
        ...
        $sql = "SELECT gibbonPerson.gibbonPersonID, title, surname, preferredName, username, gibbonRole.category
                ...
                ORDER BY preferredName, surname"; //GS//
    ...
    */
    public function createSelectStudent($name, $gibbonSchoolYearID, $params = array())
    {
        ...
        if ($params["byRoll"]) {
            if ($params["allStudents"]) {
                ...
                $sql = "SELECT gibbonPerson.gibbonPersonID, preferredName, surname, username, gibbonRollGroup.name AS name
                    ...
                    ORDER BY name, surname, preferredName"; //GS
            ...
            if ($results && $results->rowCount() > 0) {
                while ($row = $results->fetch()) {
                    if ($multipleBys) {
                        $values[__('Students by Roll Group')][$row['gibbonPersonID']] = htmlPrep($row['name']).' - '.formatName('', htmlPrep($row['preferredName']), htmlPrep($row['surname']), 'Student', true, false, $row['username']);
                    } else {
                        $values[$row['gibbonPersonID']] = htmlPrep($row['name']).' - '.formatName('', htmlPrep($row['preferredName']), htmlPrep($row['surname']), 'Student', true, false, $row['username']);
                    }
                }
            }
        }
        ...
        if ($params["byName"]) {
            if ($params["allStudents"]) {
                $sql = "SELECT gibbonPerson.gibbonPersonID, title, surname, username, preferredName, null AS name
                    FROM gibbonPerson
                        JOIN gibbonRole ON (gibbonPerson.gibbonRoleIDPrimary=gibbonRole.gibbonRoleID)
                    WHERE gibbonRole.category='Student'
                    ORDER BY surname, preferredName"; //GS
            } else {
                $data = array('gibbonSchoolYearID' => $gibbonSchoolYearID, 'date' => date('Y-m-d'));
                $sql = "SELECT gibbonPerson.gibbonPersonID, title, surname, username, preferredName, gibbonRollGroup.name AS name
                    FROM gibbonPerson
                        JOIN gibbonStudentEnrolment ON (gibbonPerson.gibbonPersonID=gibbonStudentEnrolment.gibbonPersonID)
                        JOIN gibbonRollGroup ON (gibbonStudentEnrolment.gibbonRollGroupID=gibbonRollGroup.gibbonRollGroupID)
                    WHERE status='Full'
                ...
                while ($row = $results->fetch()) {
                    if ($multipleBys) {
                        if (!$params['allStudents'] && $params['byName'] && $params['showRoll']) {
                            $values[__('Students by Name')][$row['gibbonPersonID']] = formatName(htmlPrep($row['title']), ($row['preferredName']), htmlPrep($row['surname']), 'Student', true, true, $row['username'])." (".$row['name'].")";
                        }
                        else {
                            $values[__('Students by Name')][$row['gibbonPersonID']] = formatName(htmlPrep($row['title']), ($row['preferredName']), htmlPrep($row['surname']), 'Student', true, true, $row['username']);
                        }
                    } else {
                        if (!$params['allStudents'] && $params['byName'] && $params['showRoll']) {
                            $values[$row['gibbonPersonID']] = formatName(htmlPrep($row['title']), ($row['preferredName']), htmlPrep($row['surname']), 'Student', true, true, $row['username'])." (".$row['name'].")";
                        }
                        else {
                            $values[$row['gibbonPersonID']] = formatName(htmlPrep($row['title']), ($row['preferredName']), htmlPrep($row['surname']), 'Student', true, true, $row['username']);
                        }
                    }
                }
            }
