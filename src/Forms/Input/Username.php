...
class Username extends TextField
{
    ...
    public function addGenerateUsernameButton($form)
    {
        ...
        //GS//$alertText = __('The following fields are required to generate a username:')."\n\n";
        //GS//$alertText .= __('Primary Role').', '.__('Preferred Name').', '.__('First Name').', '.__('Surname')."\n";
        $alertText = __('Can not generate username.'); //GS//

        $button = $form->getFactory()->createButton(__('Generate'));
        $button->addClass('generateUsername -ml-px')
        //GS//    ->addData('alert', $alertText)
            ->setTabIndex(-1);
